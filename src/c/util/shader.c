...
#define CHECK(p, m, d) ERROR_IF(!(p), "readBinaryFile()", (m), d, NULL)
    ...
    int pos = 0;
    {
        ...
        CHECK(fgetpos(file, (fpos_t *)&pos) == 0, "failed to get the file size.", fclose(file));
        CHECK(fseek(file, 0L, SEEK_SET) == 0, "failed to seek set.", fclose(file));
    }

    char *binary;
    {
        void *tempBinary = malloc(sizeof(char) * (size_t)pos);
        CHECK(
            fread(tempBinary, sizeof(char), (size_t)pos, file) == (size_t)pos,
            "failed to read a file.",
            { free(tempBinary); fclose(file); }
        );
        binary = (char *)tempBinary;
    }
